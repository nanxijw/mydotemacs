...
(defun backup-each-save-delete-empty-directory-recursively (dir)
  (unless (directory-empty-p dir)
    ;; clean the sub dir
    (mapc (lambda (filename)
            (unless (or (string= filename "..") (string= filename "."))
              (backup-each-save-delete-empty-directory-recursively (expand-file-name filename dir))))
          (directory-files dir)))
...
(defun backup-each-save-clean-up (day)
  (let ((substr-len (length (format-time-string backup-each-save-time-format)))
        (ti-str (format-time-string backup-each-save-time-format
                                    (time-add (current-time) (days-to-time (- 0 day))))))
    (mapc (lambda (filename)
            (when (and (not (file-directory-p filename))
                       (string< (substring filename (- 0 substr-len)) ti-str))
              (delete-file filename)))
          (directory-files-recursively backup-each-save-mirror-location ".*")))

  (backup-each-save-delete-empty-directory-recursively backup-each-save-mirror-location))
