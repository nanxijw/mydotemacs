  ...
  - 一些函数
    #+BEGIN_SRC emacs-lisp :tangle yes
      (defun add-to-load-path (dir)
        (add-to-list 'load-path (expand-file-name (file-name-concat "lisp" dir) user-emacs-directory)))
...
* 个人使用习惯方面的设置
  ...
  - 使用F3查找光标当前所在位置的symbol
    #+BEGIN_SRC emacs-lisp
      ...
      (global-set-key [C-f3] #'symbol-overlay-put)
      (global-set-key [f3] #'symbol-overlay-jump-next)
      (global-set-key [S-f3] #'symbol-overlay-jump-prev)
      (global-set-key [C-S-f3] #'symbol-overlay-remove-all)
    #+END_SRC
  ...
  - vertico
    #+BEGIN_SRC emacs-lisp
      ...
      (with-eval-after-load 'vertico
        (define-key vertico-map "\C-\l" #'vertico-directory-delete-word))
      (vertico-mode +1)
    #+END_SRC
  - consult
    #+BEGIN_SRC emacs-lisp
      ...
      (global-set-key (kbd "C-M-y") 'consult-yank-pop)
...
** P01
   ...
   - citre
    #+BEGIN_SRC emacs-lisp :tangle no
      ...
      (transient-define-prefix my-transient-citre ()
        "Citre" [("j" "Jump" citre-jump)
                 ("J" "Jump back" citre-jump-back)
                 ("u" "Update tags" citre-update-this-tags-file)
                 ("p" "Peek" citre-peek)])

      (transient-append-suffix 'my-transient-root '(0 0 -1) '("t" "Citre" my-transient-citre))
    #+END_SRC
    ctags 命令
