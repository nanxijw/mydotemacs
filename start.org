...
* 文件头
  ...
  - 解决一些命令行工具乱码问题
    #+BEGIN_SRC emacs-lisp :tangle no
      (modify-coding-system-alist 'process "[cC][mM][dD][pP][rR][oO][xX][yY]" '(utf-8 . gb18030-dos))
    #+END_SRC
  - 一些函数
    #+BEGIN_SRC emacs-lisp :tangle yes
      ...
      ;; 为一个目录中的所有 .el 文件生成 autoloads
      ;; PKG-DIR 目录
      ;; NAME 包名，最后会生成 NAME-autoloads.el 文件
      ;; (package-generate-autoloads NAME PKG-DIR)
      (autoload #'package-generate-autoloads "package")
      (defun ensure-pkg-autoloads (name &optional pkg-dir)
        (let ((auto-name (intern (concat name "-autoloads"))))
          (unless (require auto-name nil t)
            (unless pkg-dir
              (setq pkg-dir name))
            (package-generate-autoloads name (my-sub-load-path pkg-dir))
            (require auto-name))))
    #+END_SRC
  - 常量定义
    #+BEGIN_SRC emacs-lisp
...
* UI界面
  ...
  - 主题设置
    ...
    其他一些不错的主题
    #+BEGIN_SRC emacs-lisp :tangle no
      (ensure-pkg 'srcery-theme)
      (load-theme 'srcery)

      (ensure-pkg 'ef-themes)
      (load-theme 'ef-night)

      (ensure-pkg 'color-theme-sanityinc-tomorrow)
      (load-theme 'sanityinc-tomorrow-night)

      (ensure-pkg 'doom-themes)
      (load-theme 'doom-one)
  ...
  - modeline设置
    ...
    + diminish
      #+BEGIN_SRC emacs-lisp
        (ensure-pkg-autoloads "diminish" "diminish.el")
      #+END_SRC
  - 高亮当前行
    #+BEGIN_SRC emacs-lisp
  ...
  - 字体设置
    1. 使用中英文等高、等宽的字体 [[https://github.com/lxgw/LxgwWenKai][霞鹜文楷]]
       #+BEGIN_SRC emacs-lisp :tangle no
         ...
         ;; ;; Unifont使窗口宽度计算错误，会导致 (split-window-sensibly) 总是上下分割，这里修正一下
      ...
      #+BEGIN_SRC emacs-lisp :tangle yes
        (defun my/project-files-in-directory (dir)
          ...
          (let* ((default-directory dir)
                 ...
                 (command (format "fd -H -t f -0 --path-separator / -E .git/ . %s" localdir)))
    ...
    #+BEGIN_SRC emacs-lisp :tangle no
      (use-package projectile
        ...
        (setq projectile-indexing-method 'alien
              ...
              projectile-project-enable-cmd-caching nil))
  ...
  - YASnippet
    ...
    #+BEGIN_SRC emacs-lisp
      ...
      (setq yas-snippet-dirs
            (list (expand-file-name (locate-user-emacs-file "etc/snippets"))))

      (add-hook 'org-mode-hook #'yas-minor-mode)
      (add-hook 'pike-mode-hook #'yas-minor-mode)
...
* 文件备份
  ...
  - 每次保存都备份文件
    #+BEGIN_SRC emacs-lisp :tangle no
      ...
      (setq backup-each-save-time-format "%Y%m%d_%H%M")
...
* 搜索功能
  - 使用color-rg搜索
    #+BEGIN_SRC emacs-lisp :tangle yes
      ...
      (autoload #'color-rg-build-command "color-rg")
      ...
      (with-eval-after-load 'color-rg
        ...
        ;; (defun my-color-rg-build-command (oldfun &rest r)
        ;;   (let ((command-line (apply oldfun r)))
        ;;     (setq command-line (concat command-line " -Egbk"))
        ;;     command-line))
        ;; (advice-add 'color-rg-build-command :around #'my-color-rg-build-command)
        )

      ;; (modify-coding-system-alist 'process "rg" '(utf-8 . gb18030-dos))
  ...
  - vertico
    #+BEGIN_SRC emacs-lisp
      (ensure-pkg-autoloads "vertico")

      (add-to-load-path "vertico/extensions")
      (ensure-pkg-autoloads "vertico-extensions" "vertico/extensions")
   ...
   - citre
     #+BEGIN_SRC emacs-lisp :tangle no
       ...
       (use-package citre
         ...
         (setq citre-gtags-program "C:/Programs/gnuglobal653/bin/gtags.exe"
               citre-global-program "C:/Programs/gnuglobal653/bin/global.exe"
               citre-gtags-args '("--compact")
               citre-global-completion-case-sensitive nil
               citre-readtags-program "C:/Programs/ctags/readtags.exe"
               citre-ctags-program "C:/Programs/ctags/ctags.exe"
               ;; citre-project-root-function #'projectile-project-root
               citre-capf-optimize-for-popup t
               citre-default-create-tags-file-location 'global-cache
               citre-auto-enable-citre-mode-backends '(global)
               citre-auto-enable-citre-mode-modes '(pike-mode)))
     #+END_SRC
* 临时实验配置
* 快捷键设置
  - 取消一些我不用的快捷键
  ...
  - transient
    #+BEGIN_SRC emacs-lisp :tangle yes
      ...
      (transient-define-prefix my-transient-root ()
        "Root" [["One Step" ("/" "Search in project" color-rg-search-project :transient nil)]
                ["Jump to"
                 ("jf" "Function" consult-imenu :transient nil)
                 ("jl" "Line" consult-goto-line :transient nil)]

                ["Search"
                 ("s/" "In Project" color-rg-search-project :transient nil)
                ...
                ;; ["Tags"
                ;;  ("tu" "Update" citre-global-update-database :transient nil)
                ;;  ("tj" "Jump" citre-jump :transient nil)
                ;;  ("tJ" "Jump Back" citre-jump-back :transient nil)
                ;;  ("tq" "Query" (lambda ()
                ;;                  (interactive)
                ;;                  (citre-query-jump 1)) :transient nil)
                ;;  ("tp" "Peek" citre-peek :transient nil)]

                ["Buffer"
                 ("bd" "Kill" kill-current-buffer :transient nil)
                 ("bb" "List" consult-buffer :transient nil)]

                ["File"
                 ("ff" "In Project" project-find-file :transient nil)
                 ("fd" "fd" consult-fd :transient nil)
                 ("fr" "Recent" consult-recent-file :transient nil)]])
